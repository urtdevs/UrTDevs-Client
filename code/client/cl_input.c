...
qboolean CL_ReadyToSendPacket( void ) {
	...
	} else if ( net_maxpackets->integer > 125 ) {
		Cvar_Set( "net_maxpackets", "125" );
		}
		 oldPacketNum = (clc.netchan.outgoingSequence - 1) & PACKET_MASK;
	      delta = cls.realtime -  cl.outPackets[ oldPacketNum ].p_realtime;
	       if ( delta < 1000 / net_maxpackets->integer ) {
...
void CL_WritePacket( void ) {
	...
	if ( net_packetdup->integer < 0 ) {
		...
		Cvar_Set( "net_packetdup", "3" );
	 }
   oldPacketNum = (clc.netchan.outgoingSequence - 1 -  net_packetdup->integer) & PACKET_MASK;
