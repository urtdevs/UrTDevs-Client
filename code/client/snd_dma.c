...
sfxHandle_t	S_Base_RegisterSound( const char *name, qboolean compressed ) {
	...
	//@SudoKing -- workaround so servers can't crash you by sending a fake cs command with empty sound name, resulting in fatal error
	if(name[0]){
		sfx = S_FindName( name );
	} else {
		sfx = S_FindName( "sound/movers/switches/butn2.wav" ); //practically silent
	}
