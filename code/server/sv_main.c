...
static void SVC_Status( netadr_t from ) {
	...
	if ( SVC_RateLimit( &outboundLeakyBucket, 10, 100 ) ) {
		Com_DPrintf( "SVC_Status: net_rate limit exceeded, dropping request\n" );
		return;
	}
...
void SVC_Info( netadr_t from ) {
	...
	if ( SVC_RateLimitAddress( from, 10, 1000 ) ) {
		Com_DPrintf( "SVC_Info: net_rate limit from %s exceeded, dropping request\n",
			NET_AdrToString( from ) );
		return;
	}
...
int SV_RateMsec(client_t *client)
{
	...
	rateMsec = messageSize * 1000 / ((int) (net_rate * com_timescale->value));
	net_rate = Sys_Milliseconds() - client->netchan.lastSentTime;
	if(net_rate > rateMsec)
		return 0;
	else
		return rateMsec - net_rate; 
}
